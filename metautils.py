...
def infer_from_dir(dirname):
    ...
    # There may be two or more dates. If so, should the earliest for the
...
def make_pathname(path, name):
    # handle os.sep and os.altsep in name,
    # so that it does not create a subdirectory
...
def get_output_dir(rootdir, metadata):
    if 'date' in metadata:
        outdir = metadata['artist'] + \
            ' - (' + metadata['date'] + ') ' + metadata['album']
    else:
...
def get_filename(outdir, metadata):
    return make_pathname(
        outdir,
        metadata['tracknumber'] + ' - ' + metadata['title'] + '.flac')
...
def get_flac_metadata(data):
    ...
    match = re.search(
        r'^\s*tracknumber\s*=\s*([0-9]*)', data, re.MULTILINE | re.IGNORECASE)
    if match:
...
def set_meta(metadata, pathname):
    cmd = ['--set-tag=' + tag + '=' + val for tag,
           val in sorted(metadata.items())]
